...
def build_targets(pred_boxes, pred_cls, target, anchors, ignore_thres):
    ...
    #Se reemplaza por:
    obj_mask = torch.zeros(nB, nA, nG, nG, dtype=dtype_byte, device=device)
    noobj_mask = torch.ones(nB, nA, nG, nG, dtype=dtype_byte, device=device)
    class_mask = torch.zeros(nB, nA, nG, nG, dtype=dtype_float, device=device)
    iou_scores = torch.zeros(nB, nA, nG, nG, dtype=dtype_float, device=device)
    tx = torch.zeros(nB, nA, nG, nG, dtype=dtype_float, device=device)
    ty = torch.zeros(nB, nA, nG, nG, dtype=dtype_float, device=device)
    tw = torch.zeros(nB, nA, nG, nG, dtype=dtype_float, device=device)
    th = torch.zeros(nB, nA, nG, nG, dtype=dtype_float, device=device)
    tcls = torch.zeros(nB, nA, nG, nG, nC, dtype=dtype_float, device=device)
