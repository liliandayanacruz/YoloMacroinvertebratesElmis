...
class YOLOLayer(nn.Module):
    ...
    def compute_grid_offsets(self, grid_size, cuda=True):
        ...
        #Se reemplaza por:
        self.grid_x = torch.arange(g, dtype=torch.float, device='cuda' if cuda else 'cpu').repeat(g, 1).view([1, 1, g, g])
        self.grid_y = torch.arange(g, dtype=torch.float, device='cuda' if cuda else 'cpu').repeat(g, 1).t().view([1, 1, g, g])
    ...
    def forward(self, x, targets=None, img_dim=None):
        ...
        #Se reemplaza por:
        pred_boxes = torch.zeros(prediction[..., :4].shape, dtype=dtype, device=device)
